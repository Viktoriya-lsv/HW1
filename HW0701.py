...
class Student:
    ...
    def __str__(self):
        return (f'Имя: {self.name}\n'
                ...
                f'Средняя оценка за домашние задания: {sum([sum(values_list) for values_list in self.grades.values()])/sum([len(self.grades[x]) for x in self.grades])}\n'
...
class Lecturer(Mentor):
    ...
    def __str__(self):
        return (f'Имя: {self.name}\n'
                f'Фамилия: {self.surname}\n'
                f'Средняя оценка за лекции: {sum([sum(values_list) for values_list in self.grades.values()])/sum([len(self.grades[x]) for x in self.grades])}')
    def average_grade(self):
        if not self.grades:
            return 0
        total_grades = sum([sum(g) for g in self.grades.values()])
